...
처음이신가요? **[설치 가이드](docs/설치가이드.md)** 에 그림과 함께 하나씩 적어
두었습니다.
...
- 사람 많은 곳에서는 **눌러서 말하기**로 바꾸시면 됩니다. 단축키로 시작하고
  다시 눌러 끝내므로, 말하려고 마음먹은 것만 나갑니다.
...
창은 마음대로 줄이셔도 됩니다. 설정이 길어지면 세로로 넘겨 보시면 됩니다.
...
| 모델 | 고르고 받고 지웁니다. 아래 「모델 고르기」를 보세요 |
| 말하기 방식 | 자동 감지 / 눌러서 말하기 |
| 자주 쓰는 말 | 게임 낱말을 적어 두면 더 잘 알아듣습니다 |
| 고쳐 쓰기 | 자꾸 틀리는 말을 바로잡습니다 |
...
**설정** 탭의 **자주 쓰는 말** 에 게임 낱말을 쉼표로 적어 두세요. 그런 말이
나올 거라고 미리 알려 주면 훨씬 잘 알아듣습니다. 지역·NPC·직업 이름처럼
흔한 것들은 이미 들어 있습니다.

```
자주 쓰는 말 :  키아, 라비, 바리, 어비스
고쳐 쓰기    :  던젼>던전, 파뤼>파티
```

**고쳐 쓰기** 는 그래도 자꾸 틀리는 말을 보내기 직전에 바로잡습니다.

### 작은 표시창

창을 내리면 화면 위에 작게 떠 있습니다. 끌어서 옮기면 자리를 기억하고, 왼쪽
클릭으로 큰 창을 열고, 오른쪽 클릭으로 듣기를 켜고 끕니다. **방금 보낸 말도
잠깐 보여 주니**, 창을 열지 않아도 무엇이 나갔는지 알 수 있습니다.
...
있고, 한 번 받은 모델은 계속 씁니다. 판을 올려도 다시 받지 않습니다. 안 쓰는
모델은 **지우기** 로 비울 수 있습니다.
...
대화에 쓸 수 없습니다. AMD·인텔 그래픽카드는 가속에 쓸 수 없어 CPU 로 돕니다.
...
`scripts\삭제.bat` 을 두 번 누르세요. 무엇이 얼마나 지워지는지 먼저 보여 주고
물어봅니다.

| 고르기 | 지우는 것 |
|---|---|
| **전부** | 프로그램 · 내장 파이썬 · CUDA 라이브러리 · 모델 · 설정 |
| **받은 것만** | 내장 파이썬 · CUDA 라이브러리 · 모델 (프로그램과 설정은 남김) |

「받은 것만」 을 고르시면 나중에 `실행.bat` 을 누를 때 필요한 것을 새로 받습니다.
자리만 비우고 싶을 때 쓰시면 됩니다.

공용 캐시(`%USERPROFILE%\.cache\huggingface`)에 모델이 남아 있으면 따로 물어
봅니다. 그 자리는 다른 프로그램도 쓸 수 있으니 확실할 때만 지우세요.
